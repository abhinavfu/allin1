...
def how(request):
    """
    Visualizing README.md files in the website
    """
    loc = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    
    with open(file=f"{loc}/README_Vendor.md") as file:
        txt = markdown.markdown(file.read())
    ...
    def update(self, request,pk, *args, **kwargs):
        ...
        try:
            ...
            try:
                ...
                if not is_vendor and obj_PO.status == "pending":
                    data['items'] = json.loads(request.data["items"])
                elif obj_PO.status == "placed":
                    return Response({"response":"","message":"Purchase Order has placed allready."},status=status.HTTP_202_ACCEPTED)
                elif obj_PO.status == "completed" or obj_PO.acknowledgment_date != None:
                    if is_vendor:
                        return Response({"response":"","message":"Only User can rate the quality."})
                    else:
                        data['quality_rating'] = request.data["quality_rating"]
                elif obj_PO.status == "cancelled":
                    return Response({"response":"","message":"Purchase Order has cancelled allready."},status=status.HTTP_202_ACCEPTED)
            except:raise ValidationError("items must be in dictionary, eg- {'name':'item name'}")
            ...
            if PO_status == "placed" and obj_PO.status != "completed":
                if is_vendor:
            ...
            elif (PO_status == "placed" or PO_status == "cancelled") and obj_PO.status == "completed":
    ...
    def create(self, request,pk, *args, **kwargs):
        ...
        try:
            ...
            if obj_PO.vendor == vendor:
                ...
                elif obj_PO.status == "completed":
                    return Response({"response":"","message":"Purchase Order allready acknowledged."},status=status.HTTP_202_ACCEPTED)
                elif obj_PO.status == "cancelled":
                    return Response({"response":"","message":"Purchase Order has been cancelled."},status=status.HTTP_202_ACCEPTED)
                else:
                    return Response({"response":"","message":"Purchase Order has not be placed."},status=status.HTTP_202_ACCEPTED)
            else:
